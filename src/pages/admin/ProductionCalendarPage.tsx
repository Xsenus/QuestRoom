...
export default function ProductionCalendarPage() {
  ...
  const [hasYearOverride, setHasYearOverride] = useState(false);
  ...
  const loadDays = async () => {
    ...
    try {
      ...
      const safeDays = data || [];
      setDays(safeDays);
      const latestYear = safeDays.reduce((acc, day) => Math.max(acc, formatYear(day.date)), 0);
      if (latestYear && !hasYearOverride) {
        setYearFilter(latestYear.toString());
      }
    } catch (error) {
  ...
  const handleRefresh = async () => {
    const shouldRefresh = confirm('Точно обновить календарь?');
