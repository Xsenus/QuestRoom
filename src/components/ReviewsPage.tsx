...
export default function ReviewsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      if (settingsResult.reviewsMode !== 'flamp') {
        const reviewsResult = await api.getReviews(true);
        setReviews(reviewsResult || []);
      }
    } catch (error) {
      console.error('Error loading reviews/settings:', error);
    } finally {
      setLoading(false);
    }
  };
        ...
        {settings?.reviewsMode === 'flamp' ? (
          settings.reviewsFlampEmbed ? (
            ...
            <div className="text-center py-12 bg-white/10 backdrop-blur-sm rounded-lg">
              <p className="text-white text-lg">Виджет отзывов не настроен.</p>
            </div>
          )
        ) : (
          <>
