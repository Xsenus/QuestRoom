...
- `Md5Key` — секретный ключ для проверки md5 (если пустой — проверка отключается; значения из конфигурации используются только при отсутствии записи в `settings`). Можно указать несколько ключей через запятую или точку с запятой.
- `PrepayMd5Key` — секретный ключ для проверки предоплаты (если пустой — проверка отключается; значения из конфигурации используются только при отсутствии записи в `settings`). Можно указать несколько ключей через запятую или точку с запятой.
