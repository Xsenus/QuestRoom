...
- **ConnectionString**: Укажите данные подключения к Supabase PostgreSQL
- **Jwt:Key**: Сгенерируйте секретный ключ (минимум 32 символа)

```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=db.ljbgyixecqjpaqluezle.supabase.co;Port=5432;Database=postgres;Username=postgres;Password=ваш_пароль_supabase"
  },
  "Jwt": {
...
API подключается напрямую к PostgreSQL базе Supabase. Все таблицы уже созданы через миграции Supabase.
