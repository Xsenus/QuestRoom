...
- `GET /api/bookings` - Получить все бронирования (требуется авторизация admin).
  ...
  - Query параметр `searchQuery` выполняет серверный поиск по основным текстовым полям бронирования (клиент, телефон, email, агрегатор, промокод, комментарий, статус, название квеста).
...
- `POST /api/quests/{questSlug}` - Создать бронирование от агрегатора (аналогичный обработчик для интеграции Mir-kvestov.ru).
  - Принимает `application/x-www-form-urlencoded` или JSON с теми же полями.
- `your_slot_id` поддерживает GUID или числовой формат `YYYYMMDDHHMM` (по умолчанию числовой, управляется в настройках интеграции).
  - `Md5Key`/`PrepayMd5Key` могут содержать несколько ключей через запятую или точку с запятой.
